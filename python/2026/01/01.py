...
# 1. resolution_streak([[10500, 75, 15], [11000, 90, 10], [10650, 100, 9]]) 
#should return "Resolution on track: 3 day streak."
# 2. resolution_streak([[10000, 120, 5], [10950, 121, 11]]) 
#should return "Resolution failed on day 2: 1 day streak."
...
# 5. resolution_streak([[10500, 75, 15], [11000, 90, 10], [10650, 100, 9], [10200, 60, 10], [10678, 87, 9], [12431, 67, 13], [10444, 107, 19], [10111, 95, 5], [10000, 120, 7], [11980, 101, 8]]) 
#should return "Resolution on track: 10 day streak."

def resolution_streak(days):
    streak = 0
    failed = []
    for [steps, time, pages] in days:
        if steps >= 10000 and time <= 120 and pages >= 5:
            streak += 1
        else:
            failed.append(streak+1)
            # streak = 0
    if failed != []:
        return f"Resolution failed on day {failed[0]}: {streak} day streak."
    else:
