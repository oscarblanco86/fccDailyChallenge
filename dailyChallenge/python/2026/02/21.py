...
## Claude solution:def score_curling(house):
    # Assign each cell a ring distance (0 = button, 1 = ring1, 2 = ring2)
    # ring_map = {}
    # for r in range(5):
    #     for c in range(5):
    #         if r == 2 and c == 2:
    #             ring_map[(r, c)] = 0  # button (closest)
    #         elif 1 <= r <= 3 and 1 <= c <= 3:
    #             ring_map[(r, c)] = 1  # ring 1
    #         else:
    #             ring_map[(r, c)] = 2  # ring 2
    ...
    #         team = house[r][c]
    #         if team != ".":
    #             stones.append((ring_map[(r, c)], team))

    # if not stones:
    #     return "No points awarded"

    # # Find the closest distance for each team
    # red_min = min((d for d, t in stones if t == "R"), default=None)
    # yellow_min = min((d for d, t in stones if t == "Y"), default=None)

    # # If only one team has stones
    # if red_min is None and yellow_min is None:
    #     return "No points awarded"
    # if red_min is None:
    #     scoring_team, opponent_min = "Y", None
    # elif yellow_min is None:
    #     scoring_team, opponent_min = "R", None
    # elif red_min < yellow_min:
    #     scoring_team, opponent_min = "R", yellow_min
    # elif yellow_min < red_min:
    #     scoring_team, opponent_min = "Y", red_min
    # else:
    #     return "No points awarded"  # tied closest distance

    # # Count scoring team's stones closer than opponent's closest
    # if opponent_min is None:
    #     points = sum(1 for d, t in stones if t == scoring_team)
    # else:
    #     points = sum(1 for d, t in stones if t == scoring_team and d < opponent_min)

    # return f"{scoring_team}: {points}" if points > 0 else "No points awarded"
