...
def get_number_of_plants(field_size, unit, crop):
    nPlants = 0
    if crop == "corn": nPlants = field_size / 1
    if crop == "wheat": nPlants = field_size / 0.1
    if crop == "soybeans": nPlants = field_size / 0.5
    if crop == "tomatoes": nPlants = field_size / 0.25
    if crop == "lettuce": nPlants = field_size / 0.2
    if unit == 'hectares': return int(nPlants * 10000)
    if unit == 'acres': return int(nPlants * 4046.86)
